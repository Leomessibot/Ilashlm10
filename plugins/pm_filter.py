...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, FILE_REQ_CHANNEL, LOG_CHANNEL, HOW_TO_DOWNLOAD, PICS, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, DATABASE_URI, DATABASE_NAME 
...
async def pm_text(bot, message):
    ...
    k = await message.reply_text("<b>Iғ ʏᴏᴜ ᴀsᴋ ʜᴇʀᴇ ʏᴏᴜ ᴡᴏɴ´ᴛ ɢᴇᴛ ᴛʜᴇ ᴍᴏᴠɪᴇ ɢᴏ ᴛᴏ ᴍʏ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀsᴋ ᴛʜᴇɴ ʏᴏᴜ ᴡɪʟʟ ɢᴇᴛ ᴛʜᴇ ᴍᴏᴠɪᴇ ғɪʟᴇs</b>\n\n<b><i>@Cinimalokham\n@Cinimalokham\n@Cinimalokham</i></b>")
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('Cⁱⁿᵉᵐᵃ Wᵒʳˡᵈ')
            else:
...
<b>Sᴏʀʀʏ ɪ ᴏɴʟʏ ᴡᴏʀᴋ ᴏɴ</b> <b><a href='https://t.me/cinemaworld_123'>Cⁱⁿᵉᵐᵃ Wᵒʳˡᵈ</a></b><b> Gʀᴏᴜᴘ. Nᴏ ᴏᴛʜᴇʀ ᴄᴏᴍᴍᴀɴᴅ ᴡɪʟʟ ᴡᴏʀᴋ ᴏɴ ᴛʜɪs ʙᴏᴛ ᴇxᴄᴇᴘᴛ ᴘɪɴɢ. ᴅᴏɴ’ᴛ ᴡᴀsᴛᴇ ʏᴏᴜʀ ᴛɪᴍᴇ</b>""".format(
    ...
    elif query.data == "date1":
        day, date, time, time_zone_info = get_current_datetime_info()
        await query.answer(f"👋 Hᴇʟʟᴏ\n\n📅 Dᴀᴛᴇ : {date}\n🌥️ Dᴀʏ : {day}\n🌇 Uᴛᴄ : {time_zone_info}\n\n© 𝐂𝐈𝐍𝐄𝐌𝐀 𝐖𝐎𝐑𝐋𝐃", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        day, date, time = get_current_datetime_info()
        cap = TEMPLATE.format(
            query=search,
            title=imdb['title'],
            votes=imdb['votes'],
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            kk = await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(900)
            await message.delete()
            await kk.delete()
        except Exception as e:
            day, date, time, zone = get_current_datetime_info()
            kkk = await message.reply_photo(photo='https://telegra.ph/file/0a844fe2164fe700d1360.jpg',caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(900)
            await message.delete()
            await kkk.delete()
    else:
        day, date, time, zone = get_current_datetime_info()
        kkkk = await message.reply_photo(photo='https://telegra.ph/file/0a844fe2164fe700d1360.jpg', caption=cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(900)
        await message.delete()
        await kkkk.delete()
    if spoll:
